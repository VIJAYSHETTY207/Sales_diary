...
ProposalsStandardTemplate_PDF = () => 
{
  this.props.history.push({pathname:'/admin/ProposalsStandardTemplate_PDF'})
}
...
renderHeader = () => {
  return (
    ...
    <div>Lead Proposal Standard Templates</div>
    </GridItem> 
...
const { tableData, activeSuggestion,filteredSuggestions, showSuggestions,userInput, customAssignInput } = this.state;
const totalChangeAmount = Number.parseInt(this.state.change2000 * 2000) + Number.parseInt(this.state.change500  * 500) + Number.parseInt(this.state.change200  * 200) + Number.parseInt(this.state.change100  * 100) + Number.parseInt(this.state.change50  * 50) + Number.parseInt(this.state.change20  * 20) + Number.parseInt(this.state.change10  * 10) + Number.parseInt(this.state.change1  * 1);
const totalCollectedAmount = Number.parseInt(this.state.amt2000 * 2000) + Number.parseInt(this.state.amt500  * 500) + Number.parseInt(this.state.amt200  * 200) + Number.parseInt(this.state.amt100  * 100) + Number.parseInt(this.state.amt50  * 50) + Number.parseInt(this.state.amt20  * 20) + Number.parseInt(this.state.amt10  * 10) + Number.parseInt(this.state.amt1  * 1);
// getting user data
const lUserData = this.props.data;
const feeSectionwiseDetails = this.state.feeDashboardDetails;

let suggestionsListComponent;
    if (showSuggestions && userInput) {
      if (filteredSuggestions.length) {
        suggestionsListComponent = (
          <ul className="suggestions" style={{paddingLeft:'35px',paddingRight:'35px'}}>
            {filteredSuggestions.map((suggestion, index) => {
              ...
              return (
                <li key={suggestion.id} onClick={()=>this.onClickInput(suggestion.id,suggestion.value.toString())}>
                  {suggestion.value}
                </li>
              );
            })}
                ...
                <GridItem xs={12} sm={12} md={10}> 
                  <Card> 
                    ...
                    <CardText className='cardHeader' style={{width:'100%',textAlign:'center'}} >
                        ...
                        <GridItem xs={12} sm={12} md={1} > </GridItem>
                        <GridItem xs={12} sm={12} md={10}>
                          <GridItem xs={12} sm={12} md={12} style={{textAlign:'center',marginTop:'10px'}}>   
                            <React.Fragment >
                            <input className="autocomplete-input"
                            type="search"
                            placeholder="Search by Trust / Institute Name"
                            onChange={this.onChangeCustomAssignInput}
                            onKeyDown={this.onKeyDownCustomAssign}
                            value={this.state.customAssignInput}
                            />
                            {suggestionsListComponent}
                            </React.Fragment>
                          </GridItem>
                      ...
                      <GridItem xs={12} sm={12} md={12} style={{textAlign:'right'}}>
                        <div className={classes.stats}> 
                          ...
                          <button  onClick={() => this.ProposalsStandardTemplate_PDF()}  class="btn btn-info" style={{margin:'10px'}}>
                            <Print class="DocumentManagement-stats-442 svg MuiSvgIcon-root" /> Print
                          </button>
                        </div>    
                      </GridItem>
                    </GridItem>
